...
static int32_t shell_ping_take_value(FILE *fout, int32_t *value, char *param,char *message )	;
...
int32_t  Shell_ping(int32_t argc, char *argv[] )
{
    ...
    if (!print_usage)
    {
        if (argc >= 2)
        {
                        ...
                        {/* If we found something.*/
                            switch (x)
                            {
                                case (0):
                                    ...
                                    fprintf(shell_ptr->STDOUT, "num output only\n");
                                    error_flag = 0;
                                    break; /* Ignored.*/
                                case (1):
                                    i++;
                                    error_flag = shell_ping_take_value(shell_ptr->STDOUT, &(sh_ping_params[x].value), argv[i], "count");       /* Amount of ping.*/
                                    break;
                                case (2):
                                    i++;
                                    error_flag = shell_ping_take_value(shell_ptr->STDOUT, &(sh_ping_params[x].value), argv[i], "hoplimit");    /* Something like TTL(time to live)*/
                                    break;
                                case (3):
                                    i++;
                                    error_flag = shell_ping_take_value(shell_ptr->STDOUT, &(sh_ping_params[x].value), argv[i], "p_wait");      /* Time interval between two packets.*/
                                    sh_ping_params[x].value = sh_ping_params[x].value * 1000;
                                    break;
                                case (4):
                                    i++;
                                    error_flag = shell_ping_take_value(shell_ptr->STDOUT, &(sh_ping_params[x].value), argv[i], "pattern");     /* What digit we will use for packets data.*/
                                    break;
                                case (5):
                                    i++;
                                    error_flag = shell_ping_take_value(shell_ptr->STDOUT, &(sh_ping_params[x].value), argv[i], "packet size"); /* Size of packet.*/
                                    break;
                                case (6):
                                    i++;
            ...
            {
                fprintf(shell_ptr->STDOUT, "GETADDRINFO error\n");
            ...
            if(inet_ntop(addrinfo_res->ai_family, &((struct sockaddr_in6 *)((*addrinfo_res).ai_addr))->sin6_addr, addr_str, sizeof(addr_str)))
			{
                ...
                {
                    fprintf(shell_ptr->STDOUT, "Ping error - can not allocate ping buffer. Size is too big.\n");
			...
			{
		  	    fprintf(shell_ptr->STDOUT, "Address parameter is wrong.\n");
                return_code = SHELL_EXIT_ERROR;
                goto EXIT_1;
			}

            fprintf(shell_ptr->STDOUT, "Pinging %s:\n", addr_str);
            fprintf(shell_ptr->STDOUT, "Press [q] to cancel.\n");

            /* Start of sending of packets. */
            for (i=0; i< sh_ping_params[1].value; i++) /* Amount of how much packets we will send.*/
            {

               /// if (fstatus(shell_ptr->STDIN))
               /// {
               ///     unsigned char c = fgetc(shell_ptr->STDIN);
              ///      if (tolower(c)=='q')
               ///     {
              ...
              ///      }
                ...
                if (error)
                {
                    ...
                    {
                        fprintf(shell_ptr->STDOUT, "Request timed out\n");
                    }
                    else
                    {
                    #if BSP_ENET_DEVICE_COUNT > 0
                        fprintf(shell_ptr->STDOUT, "Error 0x%04lX - %s\n", error,  ENET_strerror(error));
                    #else
                        fprintf(shell_ptr->STDOUT, "Error 0x%04lX \n", error);
                    #endif
                    }
                }
                else
                {
                    ...
                    {
                        fprintf(shell_ptr->STDOUT, "Reply from [%s]: time<1ms\n", addr_str);
                    }
                    else
                    {
                        fprintf(shell_ptr->STDOUT, "Reply from [%s]: time=%ldms\n", addr_str, ping_params.round_trip_time);
                    }
                }
            ...
            if( error != MQX_OK)
            {
                fprintf(shell_ptr->STDOUT, "Ping buffer mem free error - %d \n", error);
                return_code = SHELL_EXIT_ERROR;
                goto EXIT_1;
            }
        }/* if (argc >= 2) */
    ...
    if (print_usage)
    {
        if (shorthelp)
        {
            fprintf(shell_ptr->STDOUT, "%s [-c count] [-h hoplimit] [-i interval] [-p pattern] [-s size] <host>\n", argv[0]);
        }
        else
        {
            fprintf(shell_ptr->STDOUT, "Usage: %s [-c count][-h hoplimit][-i wait][-p pattern][-s size] <host>\n", argv[0]);
        #if 0 /* TBD */
            fprintf(shell_ptr->STDOUT, "   -n numeric output only. (IPv6) \n");
        #endif
            fprintf(shell_ptr->STDOUT, "   -c Stop after sending count packets.\n");
            fprintf(shell_ptr->STDOUT, "   -h Set IPv6 hoplimit, or IPv4 TTL.\n");
            fprintf(shell_ptr->STDOUT, "   -i Wait interval seconds between sending each packet.\n");
            fprintf(shell_ptr->STDOUT, "   -p Byte pattern for data of paket.\n");
            fprintf(shell_ptr->STDOUT, "   -s Number of data bytes to be sent.\n");
        }
        goto EXIT_2;
    }
...
static int32_t shell_ping_take_value(FILE *fout, int32_t *value, char *param, char *message )
{
	...
	*value = strtol(param, &e, 0);
    if (*e == '\0')
    {
        fprintf(fout, "num. of %s= %d\n", message, (*value));
        return(0);
    }
    else
    {
        fprintf(fout, "%s is not valid number of %s\n", param, message);
        return(1);
    }
}
