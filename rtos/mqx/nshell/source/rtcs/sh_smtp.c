...
void print_usage(FILE *outstream, uint32_t short_help);
...
int32_t Shell_smtp (int32_t argc, char *argv[])
{
    ...
    if (print_help)
    {
        if (!shorthelp)
        {
            fprintf(shell_ptr->STDOUT, "Usage:"); 
        }
        fprintf(shell_ptr->STDOUT, "%s", argv[0]);
        print_usage(shell_ptr->STDOUT, shorthelp);
    ...
    do
    {
        ...
        switch (next_option)
        {
            ...
            case 'j':
                ...
                break;
            case 'h':
                print_usage(shell_ptr->STDOUT, FALSE);
                return(SHELL_EXIT_SUCCESS);
            case '?': /* The user specified an invalid option. */
                print_usage(shell_ptr->STDERR, TRUE);
    ...
    {
        fprintf(shell_ptr->STDOUT, "Required parameters not set.\n");
        print_usage(shell_ptr->STDOUT, FALSE);
    }
    ...
    if (email_text == NULL)
    {
        fprintf(shell_ptr->STDERR, "  Unable to allocate memory for email message.\n");
    ...
    if (retval != 0)
    {
        fprintf(shell_ptr->STDERR, "  getaddrinfo failed. Error: 0x%X\n", retval);
        err_code = SHELL_EXIT_ERROR;
        return(err_code);
    }
    ...
    if (rp == NULL)
    {
        fprintf(shell_ptr->STDERR, "  Unable to connect to %s.\n", server);
        err_code = SHELL_EXIT_ERROR;
    }

    if (retval != SMTP_OK)
    {
        fprintf(shell_ptr->STDOUT, "  Email sending failed%s %s\n", (strlen(errstr) > 0) ? ":":".", errstr);
        err_code = SHELL_EXIT_ERROR;
    }
    else
    {
        fprintf(shell_ptr->STDOUT, "  Email send. Server response: %s", errstr);
    }
    /* Cleanup */
    freeaddrinfo(result);
    _mem_free(errstr);
