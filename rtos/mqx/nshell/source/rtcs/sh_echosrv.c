      ...
      if (argc == 2)  {
         if (strcmp(argv[1], "start") == 0)  {
            if(0 == server_h)
            {
              ...
              if (pTemp !=  0)  {
                 fprintf(shell_ptr->STDOUT,"ECHO Server Started.\n");
                 server_h = pTemp;
              } else  {
                 fprintf(shell_ptr->STDOUT,"Unable to start ECHO Server, error = 0x%x\n",RTCS_errno);
            ...
            {
              fprintf(shell_ptr->STDOUT,"ECHO Server already running\n");
            }
            
         } else if (strcmp(argv[1], "stop") == 0)  {
            result = ECHOSRV_release(server_h);
            if (result ==  RTCS_OK)  {
               fprintf(shell_ptr->STDOUT,"ECHO Server Stopped.\n");
               server_h = 0;
            } else  {
               fprintf(shell_ptr->STDOUT,"Unable to stop ECHO Server, error = 0x%x\n",result);
               return_code = SHELL_EXIT_ERROR;
            }
         } else  {
