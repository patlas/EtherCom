...
{ /* Body */
   ...
   if (print_usage)  {
      if (shorthelp)  {
         fprintf(shell_ptr->STDOUT, "%s <ip_address>\n", argv[0]);
      } else  {
         fprintf(shell_ptr->STDOUT, "Usage: %s <ip_address> \n", argv[0]);
...
int32_t Shell_arpadd(int32_t argc, char *argv[] )
{
    ...
    if (!print_usage)
    {
        ...
        {
            fprintf(shell_ptr->STDOUT, "Invalid MAC address!\n");
            return_code = SHELL_EXIT_ERROR;
        }
        else
        {
    ...
    if (print_usage)
    {
        if (shorthelp)
        {
            fprintf(shell_ptr->STDOUT, "%s <ip_address> <mac>\n", argv[0]);
        }
        else
        {
            fprintf(shell_ptr->STDOUT, "Usage: %s <ip_address> <mac>\n", argv[0]);
            fprintf(shell_ptr->STDOUT, "   <ip_address>  = IP Address\n");
