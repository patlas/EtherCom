...
#define ERR(s, c, opterr) if(opterr){\
   char errbuf[2];\
   errbuf[0] = c; errbuf[1] = '\n';\
   fwrite(argv[0], sizeof(char), strlen(argv[0]), shell_ptr->STDERR);\
   fwrite(s, sizeof(char), strlen(s), shell_ptr->STDERR);\
   fwrite(errbuf, sizeof(char), 2, shell_ptr->STDERR);}
...
int Shell_get_current_filesystem(void *argv)
{
   SHELL_CONTEXT_PTR    shell_ptr = Shell_get_context( argv );
...
int32_t Shell_set_current_filesystem(void *argv, int fd)
{
   SHELL_CONTEXT_PTR    shell_ptr = Shell_get_context( argv );
...
int32_t Shell_set_current_filesystem_by_name(void *argv, char *dev_name)
{
   ...
   fd = _io_get_fs_by_name(dev_name);
