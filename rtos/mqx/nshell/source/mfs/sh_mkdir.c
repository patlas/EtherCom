...
{ /* Body */
      ...
      if (argc !=  2) {
         ...
         else
         {
            ...
            if (0 > fd)  {
               ...
               if(!error)
               {
                  error = ioctl(fd, IO_IOCTL_CREATE_SUBDIR, (void *) abs_path);
               }
               
               if (0 > error)  {
                  fprintf(shell_ptr->STDOUT, "Error creating directory %s\n", argv[1]);
               }
            }
   ...
   if (print_usage)  {
      if (shorthelp)  {
         ...
         fprintf(shell_ptr->STDOUT, "   <directory> = name of directory to create\n");
      }
   }
   return return_code;
}
