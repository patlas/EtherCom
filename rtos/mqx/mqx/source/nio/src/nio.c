...
/** Internal function to call ioctl.
 */
static int _nio_ioctl_wrap(NIO_IOCTL_FPTR ioctl_f, void *dev_context, void *fp_context, unsigned long int request, ...)
{
    ...
    va_start(ap, request);
    if (ioctl_f){
        res = ioctl_f(dev_context, fp_context, request, ap);
    }
    else{
        errno = ENOTSUP;
    }
...
int _nio_init(int max_fd) {
    ...
    if (NULL != FD_TABLE) {
        ...
        // reset device list
        DEV_LIST.ITEM = NULL;
        OSA_SemaCreate(&DEV_LIST.LOCK, 1);

        // create semaphore for locking FD_TABLE
        OSA_SemaCreate(&FD_TABLE->LOCK, 1);
    ...
    if (0 == dev_check_name(name)) {
        ...
        if (NULL == dev) {
            ...
            if (NULL != dev) {
                ...
                OSA_SemaCreate(&dev->LOCK, 1);

                // call initialization fn
                if (dev->FN->INIT)
                {
                    res = dev->FN->INIT(init_data, &dev->CONTEXT);
                }
        ...
        while (dev->USAGE) {
            ...
            if ((NULL != fp) && (0 < fp->USAGE)) {
                ...
                if (dev->FN->IOCTL) {
                    ...
                    /* cretae dummy va_list variable */
                    _nio_ioctl_wrap(dev->FN->IOCTL, dev->CONTEXT, fp->CONTEXT, IOCTL_ABORT);
                }
            }
            else
            {
            ...
            if (fp) {
                // call low level close - dealloc fp context
                dev->FN->CLOSE(dev->CONTEXT, fp->CONTEXT);
...
static int _nio_open_intern(NIO_DEV_STRUCT *dev, const char *name, int flags, int mode) {
    void *fp_context;
    NIO_FILE *fp;
    int fd = -1;
    ...
    if (NULL != dev) {
        if (NULL != dev->FN->OPEN) {
            ...
            if (dev->FN->OPEN(dev->CONTEXT, name, flags, &fp_context)) {
                // error, exception
                // free resourcess
                lock_fd_table();
                FD_TABLE->FP[fd] = NULL;
...
int _nio_open(const char *name, int flags, ...) {
    ...
    if (dev) {
        ...
        fd = _nio_open_intern(dev, name, flags, mode);
        if (0 > fd) {
...
int _nio_close(int fd) {
    ...
    if (NULL != dev) {
        ...
        if (NULL != dev->FN->CLOSE) {
            if (0 < dev->FN->CLOSE(dev->CONTEXT, fp->CONTEXT)) {
                // error, exception
                errno = err = EIO;
            }
            else {
...
ssize_t _nio_read(int fd, void *buf, size_t nbytes) {
    ...
    // call real read
    if (dev->FN->READ) {
        res = dev->FN->READ(dev->CONTEXT, fp->CONTEXT, buf, nbytes);
    }
    else {
        errno = ENOTSUP;
    }
...
ssize_t _nio_write(int fd, const void *buf, size_t nbytes) {
    ...
    // call real write
    if (dev->FN->WRITE) {
        res = dev->FN->WRITE(dev->CONTEXT, fp->CONTEXT, buf, nbytes);
    }
    else {
        errno = ENOTSUP;
    }
...
int _nio_ioctl(int fd, unsigned long int request, ...) {
    ...
    if (dev->FN->IOCTL) {
        res = dev->FN->IOCTL(dev->CONTEXT, fp->CONTEXT, request, ap);
    }
    else {
        errno = ENOTSUP;
    }
...
off_t _nio_lseek(int fd, off_t offset, int whence) {
    ...
    if (dev->FN->LSEEK) {
        res = dev->FN->LSEEK(dev->CONTEXT, fp->CONTEXT, offset, whence);
    }
    else {
        errno = ENOTSUP;
    }
